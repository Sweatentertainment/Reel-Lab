...
/* three headline figures across the slide — the results-at-a-glance treatment */
export const bigThree = ({ section, label, headline = '', items, foot = '' }) => ({
  section,
  grain: 'soft',
  html: `
    <div class="pad l-mid">
      ${label ? `<div class="label reveal" style="font-size:18px;margin-bottom:${headline ? 32 : 60}px">${label}</div>` : ''}
      ${headline ? `<h2 class="display reveal" style="--d:.08s;font-size:64px;margin-bottom:78px">${headline}</h2>` : ''}
      <div class="reveal" style="--d:.18s;display:grid;grid-template-columns:repeat(3,1fr);gap:70px">
        ${items
          .map(
            (i, n) => `
          <div style="--d:${0.18 + n * 0.1}s">
            <div class="display" style="font-size:108px;line-height:0.98;color:var(--blue)">${i.v}</div>
            <div class="label" style="font-size:18px;margin:26px 0 14px">${i.k}</div>
            <p class="body" style="font-size:21px;max-width:400px">${i.note}</p>
          </div>`,
          )
          .join('')}
      </div>
      ${foot ? `<p class="body reveal" style="--d:.5s;font-size:21px;max-width:1000px;margin-top:80px;opacity:0.75">${foot}</p>` : ''}
    </div>`,
});

/* a numbered sequence — the method, not a result */
export const ladder = ({ section, label, headline, steps, foot = '' }) => ({
  section,
  grain: 'soft',
  html: `
    <div class="pad l-mid">
      <div class="label reveal" style="font-size:18px;margin-bottom:30px">${label}</div>
      <h2 class="display reveal" style="--d:.08s;font-size:62px;margin-bottom:60px">${headline}</h2>
      <ol class="steps reveal" style="--d:.2s">
        ${steps
          .map(
            (s, n) => `
          <li><span>${String(n + 1).padStart(2, '0')}</span>
            <div>
              <div style="font-family:var(--sans);font-weight:700;font-size:27px;letter-spacing:-0.03em;margin-bottom:10px">${s.t}</div>
              <p class="body" style="font-size:20px;max-width:620px">${s.d}</p>
            </div>
          </li>`,
          )
          .join('')}
      </ol>
      ${foot ? `<p class="body reveal" style="--d:.44s;font-size:21px;max-width:1000px;margin-top:56px;opacity:0.75">${foot}</p>` : ''}
    </div>`,
});

/* black, grainy, bracketed — the section-title treatment */
export const bracketTitle = ({ section, text, size = 132 }) => ({
  section,
  grain: true,
  html: `
    <div class="pad l-centre">
      <h2 class="display reveal" style="font-size:${size}px"><span class="bracket">${text}</span></h2>
    </div>`,
});

/* the closing call to action */
export const closer = ({ section, headline, body }) => ({
  section,
  grain: 'soft',
  html: `
    <div class="pad l-centre">
      <h2 class="display reveal" style="font-size:104px;margin-bottom:44px">${headline}</h2>
      <p class="body reveal" style="--d:.16s;font-size:26px;max-width:860px;text-align:center">${body}</p>
      <p class="reveal" style="--d:.3s;font-size:30px;font-weight:700;margin-top:56px">
